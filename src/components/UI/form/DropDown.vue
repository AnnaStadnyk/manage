<template>
  <Listbox :modelValue="modelValue" @update:modelValue="(value) => this.$emit('update:modelValue', value)" by="id">
    <ListboxLabel :for="uuid" v-if="label" class="block mb-3 lg:mb-4 text-sm text-headline font-semibold">{{ label }}
    </ListboxLabel>
    <div class="relative">
      <ListboxButton @focus="this.$emit('cancel-error')"
        class="relative w-full cursor-default outline-0 border-2 rounded-full bg-white py-3 xl:py-3.5 px-8 text-left focus:border-brand focus-visible:border-brand transition duration-300 ease-in"
