...
<template>
  <header ref="headerEl" class="relative">
    <div
      class="fixed w-full py-5 lg:py-8 transition-all duration-300 ease-in z-40"
      :class="{
        'before:absolute before:top-0 before:left-0 before:w-full before:h-full before:bg-white before:-z-10':
          !isScroll,
        'pt-3 pb-3 lg:pt-3.5 lg:pb-3.5': !isScroll
      }"
    >
      <div class="container flex gap-4 justify-between items-center">
        ...
        <TheButton @click="toogleContact" v-show="!isMobile" mode="brand" isShadow="true"
          >Get Started
        </TheButton>
        <div
          v-show="isMobile"
          class="flex flex-col h-8 justify-between relative z-40"
          @click="toggleMobile"
          ref="icon-mobile"
          id="icon-mobile"
        >
          <span
            v-for="i in 3"
            class="block w-8 h-1 bg-headline transition duration-300 ease-in"
            :class="{
        ...
        >
          <nav
            ...
            ref="mobile-menu"
            id="mobile-menu"
            class="absolute right-0 max-w-xs w-full top-0 h-screen bg-brand-light overflow-auto flex flex-col gap-8 md:gap-10 items-center"
          >
